...
def is_district(location_part):
    normalized_location_part = (
        location_part.strip().lower()
    )  # Normalize the input to lower case
    return any(
        district.lower() == normalized_location_part for district in districts_in_nepal
    )
...
if ner_button and st.session_state["location"] and st.session_state["hazard_type"]:
    # Filtering valid districts
    valid_districts = [
        loc for loc in st.session_state["location"].split(", ") if is_district(loc)
    ]
    if valid_districts:
        visualization_data = {
            "location": ", ".join(valid_districts),
    ...
    # UNCOMMENT TO PREVIOUS VERSION
    # if "process_button" in locals() and process_button:
    # results = []
    # map_folium = None  # We will create a single map and add all data to it
    ...
    #         )
    #         hazard_types = ", ".join(
    #             set(ner_result["results"]["entities"].get("hazard_type", []))
    #         )

    #         # Generate map for each location from CSV
    #         for location, hazard_type, date in zip(
    #             locations.split(", "), hazard_types.split(", "), dates.split(", ")
    #         ):
    #             map_data = {
    #                 "location": location,
    #                 "date": date,
    ...
    #             }
    #             if not map_folium:
    #                 map_folium = generate_disaster_map(
    #                     map_data, "Disaster Layer", "Disaster Data"
    #                 )
    #             else:
    #                 # Now adds to the existing map
    #                 map_folium = generate_disaster_map(
    #                     map_data,
    #                     "Disaster Layer",
    #                     "Disaster Data",
    #                     existing_map=map_folium,
    #                 )
    ...
    # if map_folium:
    #     folium_static(map_folium)  # Display the final map with all locations
...
if "process_button" in locals() and process_button:
    ...
    for text in data["text"]:
        classification_result = process_text("http://0.0.0.0:5000/predict", text)
        ner_result = process_text("http://0.0.0.0:5000/extract", text)
        print(f"ner_results: {ner_result}, type: {type(ner_result)}")

        if ner_result:
            print(
                "Raw locations:", ner_result["results"]["entities"].get("location", [])
            )
            # Filter out non-district locations and 'nan' values
            valid_locations = filter(
                lambda x: x != "nan" and is_district(x),
                ner_result["results"]["entities"].get("location", []),
            )
            locations = ", ".join(set(valid_locations))
            ...
            # Generate map for each valid location from CSV
            for location, hazard_type, date in zip(
                locations.split(", "), hazard_types.split(", "), dates.split(", ")
            ):
                if location:  # Ensures that only valid locations are processed
                    map_data = {
                        "location": location,
                        "date": date,
                        "hazard_type": hazard_type,
